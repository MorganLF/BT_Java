...
public class Account {
    ...
    public double napTien() {
        double nap;
        System.out.print("Nhập số tiền bạn cần nạp vô: ");
        nap = sc.nextDouble();
        if (nap >= 0) {
            soTienTrongTK = nap + soTienTrongTK;
            NumberFormat currencyEN = NumberFormat.getCurrencyInstance();
            String str1 = currencyEN.format(nap);
            System.out.println("bạn vừa nạp thành công " + str1 + " vào tài khoản của bạn.");
        } else {
    ...
    public double rutTien() {
        double phi = 5;
        double rut;
        System.out.print("Nhập số tiền bạn cần rút ra: ");
        rut = sc.nextDouble();
        if (rut <= (soTienTrongTK - phi)) {
            soTienTrongTK = soTienTrongTK - (rut + phi);
            NumberFormat currencyEN = NumberFormat.getCurrencyInstance();
            String str1 = currencyEN.format(rut);
            System.out.println("Bạn vừa rút ra thành công " + str1 + "Đ từ tài khoản. Phí là $1.");
        } else {
            System.out.println("Số tiền muốn rút không thành công do số tiền không hợp lệ");
            return rutTien();
        }
        return rut;
    }
    public double daoHan() {
        ...
        NumberFormat currencyEN = NumberFormat.getCurrencyInstance();
        String str1 = currencyEN.format(soTienTrongTK);
        System.out.println("Bạn vừa nhận được " + str1 + " từ đáo hạn 1 tháng");
